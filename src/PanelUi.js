function getLdvPanelHtml() {
  ...
  return [
    ...
    'var renderer=null;',
    'function routeDashboard(d){if(currentView==="dashboard")renderer.render("dashboard",d);else if(currentView==="alerts")renderer.render("alerts",d);}',
    'function resolveConvocation(matchId){renderer.dispatch({type:"resolveConvocationForMatch",matchId:matchId});}',
    'function openConvocations(){var id=panelState.selectedProgrammedMatchId||((panelState.referenceData.programmedMatches||[])[0]||{}).partidoId||"";if(id)resolveConvocation(id);else renderer.render("convocations");}',
    'function openPostMatch(){var id=panelState.selectedPlayedMatchId||((panelState.referenceData.playedMatches||[])[0]||{}).partidoId||"";renderer.render("postmatch");if(id)controller.loadPostMatch(id);}',
    'var controller=createPanelClientController({callServer:run,state:panelState,render:{dashboard:routeDashboard,referenceData:function(){if(currentView==="matches")renderer.render("matches");else if(currentView==="convocations")openConvocations();else if(currentView==="postmatch")openPostMatch();},attendance:function(d){renderer.render("attendance",d);},convocation:function(d){renderer.render("convocations",d);},postMatch:function(d){renderer.render("postmatch",d);},matchWrite:function(){controller.loadReferenceData(function(){controller.loadDashboard();renderer.render("matches");});},convocationWrite:function(d){controller.loadReferenceData(function(){if(d&&d.CONVOCATORIA_ID)controller.loadConvocation(d.CONVOCATORIA_ID);else openConvocations();});},communicationWrite:function(){controller.loadReferenceData(function(){renderer.render("convocations");});},participationWrite:function(){controller.loadPostMatch(panelState.selectedPlayedMatchId);},error:function(m){document.getElementById("panel-error").textContent=m;}}});',
    'renderer=createPanelRenderer({state:panelState,controller:controller,document:document});',
    'function load(view){currentView=view;document.getElementById("view-title").textContent=view; if(view==="dashboard"){controller.loadDashboard();controller.loadReferenceData();} else if(view==="attendance"){controller.loadReferenceData(function(){controller.loadAttendance();});} else if(view==="matches"){controller.loadReferenceData(function(){renderer.render("matches");});} else if(view==="convocations"){controller.loadReferenceData(function(){openConvocations();});} else if(view==="postmatch"){controller.loadReferenceData(function(){openPostMatch();});} else if(view==="alerts"){controller.loadDashboard();renderer.render("alerts");}}',
    ...
    'document.addEventListener("click",function(e){var t=e.target,a=t.getAttribute("data-action");if(!a)return;if(a==="mark-attendance")renderer.dispatch({type:"markAttendance",studentId:t.getAttribute("data-student-id"),state:t.getAttribute("data-state")});if(a==="resolve-absence"){var r=document.querySelector(".absence-reason[data-attendance-id=\\""+t.getAttribute("data-attendance-id")+"\\"]");renderer.dispatch({type:"resolveAbsence",attendanceId:t.getAttribute("data-attendance-id"),targetState:t.getAttribute("data-target-state"),reason:r&&r.value});}if(a==="match-cancel")renderer.dispatch({type:"cancelMatch",matchId:t.getAttribute("data-match-id")});if(a==="convocation-generate")renderer.dispatch({type:"generateConvocation",matchId:t.getAttribute("data-match-id")});if(a==="convocation-approve"){var approvalActor=document.getElementById("convocation-approval-actor");try{renderer.dispatch({type:"approveConvocation",convocationId:t.getAttribute("data-convocation-id"),actor:approvalActor&&approvalActor.value});}catch(error){document.getElementById("panel-error").textContent=String(error&&error.message?error.message:error).split(":")[0];}}if(a==="communication-prepare")renderer.dispatch({type:"prepareCommunications",convocationId:t.getAttribute("data-convocation-id")});if(a==="communication-send")renderer.dispatch({type:"sendPendingCommunications"});if(a==="participation-save")renderer.dispatch({type:"saveParticipationFromElement",button:t});});',
    'document.addEventListener("submit",function(e){var a=e.target.getAttribute("data-action");if(!a)return;e.preventDefault();if(a==="match-create")renderer.dispatch({type:"createMatch",payload:e.target});if(a==="match-update")renderer.dispatch({type:"updateMatch",matchId:e.target.getAttribute("data-match-id"),payload:e.target});if(a==="match-played")renderer.dispatch({type:"markMatchPlayed",matchId:e.target.getAttribute("data-match-id"),payload:e.target});});',
