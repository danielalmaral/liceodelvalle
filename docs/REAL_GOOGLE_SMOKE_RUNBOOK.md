...
3. Crear el proyecto Apps Script ligado a ese Spreadsheet.
4. Configurar Script Properties:
   - `LDV_SPREADSHEET_ID`
   - `LDV_EXTERNAL_MAIL_ENABLED=FALSE`
5. Mantener correo externo deshabilitado.
6. Configurar `clasp` con el `scriptId` del proyecto ligado al Spreadsheet.

## Ejecucion Controlada

1. Hacer `clasp push` controlado contra el `scriptId` container-bound desde revision certificada.
...
6. Abrir `PANEL` desde el menu/sidebar del Spreadsheet solo despues de que CONFIG este listo.
